...
$(function() {
	...
	function formatDatetime() {
		$('time[datetime]').each(function() {  /* 각 시간 DOM마다 */
			const time = $(this);
			const date = new Date(time.attr('datetime'));  /* DOM의 시간 ISO 문자열 */
			if(!date) return;  /* 없으면 되돌아간다. */
			
			const now = new Date();  /* 현재 시간 */
			const fmt = time.attr('data-format');  /* 포맷할 형식 */
			const floorof = Math.floor;  /* 소수점 이하 버리기 함수 */
			
			if(fmt && fmt !== 'b') {  /* Y, m, d, H, i, s에 날짜와 시각을 대입한다. */
				var ret = fmt;
				...
				var ret = '';  /* 결과값 */
				const gap = (now.getTime() - date.getTime()) / 1000;  /* 현재시각과 DOM 시각의 차이(초) */
				
				if(gap / 31104000 >= 1) {  /* 1년 이상 차이나면 n년 전 */
				...
				else {  /* 1초 미만이라면 그냥 방금전 */
					ret = '방금 전';
				}
				
				time.text(ret);
				time.attr('title',  /* 마우스 올리면 실제 시각 출력 */
					date.getFullYear() + '년 ' + 
					(date.getMonth() + 1) + '월 ' + 
					date.getDate() + '일 ' + 
	...
	window.transition = function(oldDiv, newDiv) {
		newDiv.fadeTo(1, 0, nevermind);  /* 다음 페이지는 일단 안 보이게 한다. */
		oldDiv.css('transition', 'all 0.45s');  /* 전환 듀레이션 */
		newDiv.css('transition', 'all 0.45s');
		
		oldDiv.css('transform', 'scale(3)');  /* 기존 페이지를 확대하면서 */
		oldDiv.fadeTo(0, 0, () => {  /* 페이드아웃시킨다. */
			setTimeout(() => oldDiv.hide(), 450);
			
			newDiv.fadeTo(50, 1, nevermind);  /* 다음 페이지를 페이드 인하며 */
			newDiv.css('transform', 'scale(1)');  /* 보통 크기로 만든다. */
		});
	};
	...
	const valueChange = 'propertychange change keyup paste input';  /* 입력값 변경 이벤트 */
	
	/* 알림 풍선                      제목    내용  표시시간 alert여부 색상 */
	window.alertBalloon = function(title, content, delay, noMsgbox, type) {
		delay    = delay || 3000;
		noMsgbox = noMsgbox || 0;
		type     = type || 'normal';
		
		if(compatMode2) {  /* 오래된 브라우저는 그냥 alert로 출력한다 */
			if(!noMsgbox) alert(title + ' ' + content);
		} else {
			$('.js-alert-balloon').remove();  /* 기존 풍선 삭제 */
			...
			const light = balloon.find('> img.light-arrow');  /* 애니메이션 불빛 */
			...
			}).prependTo('body');  /* 페이지 상단에 넣기 */
			
			var width = balloon.width();  /* 풍선 너비 */
			setTimeout(function() {  /* 페이드 인이 1/3쯤 됐을 때 불빛을 움직이다. */
				light.animate({
					'left': width  /* 풍선 끝 부분으로 이동 */
				}, 400, 'linear', function() {
					light.fadeOut(200, function() {  /* 다 왔으면 페이드 아웃시킨다. */
						light.remove();
					});
				});
			}, 120);
			...
			balloon.fadeIn(300, function() {
				if(delay > 0) {  /* 딜레이가 있으면  */
					setTimeout(function() {
						balloon.fadeOut(300, function() {  /* 지정한 시간 후 페이드 아웃. */
							balloon.remove();
						});
					}, delay);
				}
	...
	window.confirmBalloon = function(title, content, callbackOK, callbackCancel, y, n) {
		...
		if(compatMode2) {  /* 오래된 브라우저는 그냥 confirm로 출력. */
			...
			balloon.find('> p > span > button.yes-btn').click(function() {  /* 예를 누르면 콜백 실행 */
				callbackOK();
	...
	$('.js-alert-balloon a.alert-balloon-close').click(function() {
		$(this).parent().parent().fadeOut(300, function() {  /* 닫기 누르면 페이드 아웃 */
			$(this).remove();
		});
	});
	
	if(typeof ActiveXObject == 'function' && !document.addEventListener) {  /* IE 호환 */
	...
	$('.dropdown-search').on(valueChange, function() {
	...
	const loadingRes2 = loadingRes + '[data-locked="false"]';
	...
	if (
		...
		function fetchComments(tnum) {
			...
			if($(loadingRes2).length) {
				const loadingID = $(loadingRes2)[0].getAttribute('data-id');
