...
wiki.post(/\/move\/(.*)/, async (req, res) => {
	...
	if(title.startsWith('사용자:') || newtitle.startsWith('사용자:')) {
		return res.send(await showError(req, 'invalid'));
	}
	
	if(!title.startsWith('파일:') && newtitle.startsWith('파일:')) {
		return res.send(await showError(req, 'invalid'));
	}
	...
	if(!await getacl(req, newtitle, 'read')) {
		return res.send(await showError(req, 'insuffisient_privileges_read'));
	}
	
	if(!await getacl(req, newtitle, 'edit')) {
		return res.send(await showError(req, 'insuffisient_privileges_edit'));
	}
	
	if(!await getacl(req, newtitle, 'move')) {
		return res.send(await showError(req, 'insuffisient_privileges_move'));
	}
