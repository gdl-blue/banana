wiki.get(/^\/acl\/(.*)/, async function aclControlPanel(req, res) {
	...
	switch(acltyp) {
		case 'action-based':
			...
			var content = `
				<style>
					${(req.query['nojs'] == '1' || (!req.query['nojs'] && compatMode(req))) || !getperm(req, 'acl', ip_check(req)) ? '.acl-controller {display: none; }' : ''}
				</style>
			`;
			...
			if(req.query['nojs'] == '1' && getperm(req, 'acl', ip_check(req))) {
...
wiki.post(/^\/acl\/(.*)/, async function setACL(req, res) {
	...
	switch(acltyp) {
		case 'action-based':
			...
			if(!getperm(req, 'acl', ip_check(req))) {
