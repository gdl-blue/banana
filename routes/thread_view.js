wiki.get('/thread/:tnum', async function viewThread(req, res) {
	...
	if((deleted = curs.fetchall().length) && !getperm(req, 'developer', ip_check(req))) {
	...
	var dbdata = await curs.execute("select title, topic, status, type, ncontent, ocontent, baserev from threads where tnum = ? and ((subwikiid = ? and not subwikiid = 'global') or subwikiid = 'global')", [tnum, subwiki(req)]);
	...
	const hiddendata = await curs.execute("select hidden from res where tnum = ? and ((subwikiid = ? and not subwikiid = 'global') or subwikiid = 'global') order by cast(id as integer) asc", [tnum, subwiki(req)]);
	...
	if(getperm(req, 'update_thread_status', ip_check(req)) && type != 'edit_request') {
		var sts = '';
		...
		if(getperm(req, 'admin', ip_check(req))) {
			content += `
	...
	if(getperm(req, 'update_thread_document', ip_check(req)) && type != 'edit_request') {
		content += `
...
wiki.post('/thread/:tnum', async function postThreadComment(req, res) {
	...
	await curs.execute("select id from res where tnum = ? and ((subwikiid = ? and not subwikiid = 'global') or subwikiid = 'global') order by cast(id as integer) desc limit 1", [tnum, subwiki(req)]);
	...
	await curs.execute("insert into res (id, content, username, time, hidden, hider, status, tnum, ismember, isadmin, subwikiid) \
					values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [
						String(lid + 1), req.body['text'], ip_check(req), getTime(), '0', '', '0', tnum, islogin(req) ? 'author' : 'ip', getperm(req, 'admin', ip_check(req)) || getperm(req, 'fake_admin', ip_check(req)) ? '1' : '0', (sid ? 'global' : subwiki(req))
					]);
					
	curs.execute("update threads set time = ? where tnum = ? and ((subwikiid = ? and not subwikiid = 'global') or subwikiid = 'global')", [getTime(), tnum, subwiki(req)]);
