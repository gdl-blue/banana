wiki.get(/\/revert\/(.*)/, async (req, res) => {
	...
	const _recentRev = await curs.execute("select content, rev from history where title = ? and ((title like '사용자:%') or (subwikiid = ? and not title like '사용자:%')) order by cast(rev as integer) desc limit 1", [title, subwiki(req)]);
	if(!_recentRev.length) {
		return res.send(await showError(req, 'document_not_found'));
	}
	
	const dbdata = await curs.execute("select content, time, username from history where title = ? and rev = ? and ((title like '사용자:%') or (subwikiid = ? and not title like '사용자:%'))", [title, rev, subwiki(req)]);
	if(!dbdata.length) {
	...
	var content = `
		...
		<form>
			...
			<ul class="nav nav-tabs" style="height: 38px;">
				<li class=nav-item>
					<a class="nav-link active" href="#preview">미리보기</a>
				</li>
				
				<li class=nav-item>
					<a class=nav-link href="#raw">날내용</a>
				</li>
				
				<li class=nav-item>
					<a class=nav-link href="#diff">비교</a>
				</li>
			</ul>
...
;
	...
	await curs.execute("update documents set content = ? where title = ? and ((title like '사용자:%') or (subwikiid = ? and not title like '사용자:%'))", [revdata.content, title, subwiki(req)]);
	...
	curs.execute("insert into history (title, content, rev, username, time, changes, log, iserq, erqnum, ismember, advance, subwikiid) \
					values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [
		title, revdata.content, String(Number(recentRev.rev) + 1), ip_check(req), getTime(), (rawChanges > 0 ? '+' : '') + rawChanges, req.body['log'], '0', '-1', islogin(req) ? 'author' : 'ip', '(' + rev + '판으로 복원)', (title.startsWith('사용자:') ? '' : subwiki(req))
	]);
