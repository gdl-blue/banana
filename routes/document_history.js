wiki.get(/^\/history\/(.*)/, async function viewHistory(req, res) {
	...
	if(from) {
		dbdata = await curs.execute("select rev, time, changes, log, iserq, erqnum, advance, ismember, username from history \
						where title = ? and (cast(rev as integer) <= ? AND cast(rev as integer) > ?) and ((title like '사용자:%') or (subwikiid = ? and not title like '사용자:%')) \
						order by cast(rev as integer) desc",
						[title, Number(from), Number(from) - 30, (title.startsWith('사용자:') ? '' : subwiki(req))]);
	} else if(until) {
		dbdata = await curs.execute("select rev, time, changes, log, iserq, erqnum, advance, ismember, username from history \
						where title = ? and (cast(rev as integer) >= ? AND cast(rev as integer) < ?) and ((title like '사용자:%') or (subwikiid = ? and not title like '사용자:%')) \
						order by cast(rev as integer) desc",
						[title, Number(until), Number(until) + 30, (title.startsWith('사용자:') ? '' : subwiki(req))]);
		...
		switch(target.toLowerCase()) {
			case 'username':
				dbdata = await curs.execute("select rev, time, changes, log, iserq, erqnum, advance, ismember, username from history \
						where title = ? and username = ? and ((title like '사용자:%') or (subwikiid = ? and not title like '사용자:%')) order by cast(rev as integer) desc limit 1000 COLLATE NOCASE",
						[title, query, (title.startsWith('사용자:') ? '' : subwiki(req))]);
			break; case 'count':
				if(isNaN(Number(query))) return res.send(await showError(req, 'invalid_value'));
				dbdata = await curs.execute("select rev, time, changes, log, iserq, erqnum, advance, ismember, username from history \
						where title = ? and cast(changes as integer) = ? and ((title like '사용자:%') or (subwikiid = ? and not title like '사용자:%')) order by cast(rev as integer) desc limit 1000",
						[title, Number(query), (title.startsWith('사용자:') ? '' : subwiki(req))]);
			...
			break; case 'editrequest':
				dbdata = await curs.execute("select rev, time, changes, log, iserq, erqnum, advance, ismember, username from history \
						where title = ? and iserq = '1' and ((title like '사용자:%') or (subwikiid = ? and not title like '사용자:%')) order by cast(rev as integer) desc limit 1000",
						[title, (title.startsWith('사용자:') ? '' : subwiki(req))]);
			break; case 'log':
				dbdata = await curs.execute("select rev, time, changes, log, iserq, erqnum, advance, ismember, username from history \
						where title = ? and log like ? and ((title like '사용자:%') or (subwikiid = ? and not title like '사용자:%')) order by cast(rev as integer) desc limit 1000 COLLATE NOCASE",
						[title, query.replace('*', '%').replace('?', '_'), (title.startsWith('사용자:') ? '' : subwiki(req))]);
			break; case 'logmatch':
				dbdata = await curs.execute("select rev, time, changes, log, iserq, erqnum, advance, ismember, username from history \
						where title = ? and log = ? and ((title like '사용자:%') or (subwikiid = ? and not title like '사용자:%')) order by cast(rev as integer) desc limit 1000 COLLATE NOCASE",
						[title, query.replace('*', '%').replace('?', '_'), (title.startsWith('사용자:') ? '' : subwiki(req))]);
			break; default:
		...
		dbdata = await curs.execute("select rev, time, changes, log, iserq, erqnum, advance, ismember, username from history \
						where title = ? and ((title like '사용자:%') or (subwikiid = ? and not title like '사용자:%')) order by cast(rev as integer) desc limit 30",
						[title, (title.startsWith('사용자:') ? '' : subwiki(req))]);
	}
