...
function get_post() {
    ...
    check = document.getElementById('noClippy');
    if(check.checked === true) {
        document.cookie = 'noClippy=1;';
    } else {
        document.cookie = 'noClippy=0;';
    }
    ...
    check = document.getElementById('agentChar');
    if(check.value === 'Rover') {
        document.cookie = 'agentChar=Rover;';
    } else if(check.value === 'Merlin') {
        document.cookie = 'agentChar=Merlin;';
    } else if(check.value === 'Bonzi') {
        document.cookie = 'agentChar=Bonzi;';
    } else if(check.value === 'Rocky') {
        document.cookie = 'agentChar=Rocky;';
    }
...
function main_load() {
    ...
    if(
        cookies.match(regex_data('noClippy')) &&
        cookies.match(regex_data('noClippy'))[1] === '1'
    ) {
        head_data.innerHTML += '<style>.clippy {display: none !important; }</style>';
        $('.clippy').remove();
    }
...
$(function () {
    if(window.location.pathname === '/settings') {
        ...
        if(cookies.match(regex_data('agentChar'))) {
            if(cookies.match(regex_data('agentChar'))[1] === 'Rover') {
                agentTypeOptions = ' \
                    <option value=Rover selected>똘똘이</option> \
                    <option value=Merlin>사이버 마법사</option> \
                    <option value=Bonzi>Bonzi-Buddy</option> \
                    <option value=Rocky>재롱이</option> \
                ';
            } else if(cookies.match(regex_data('agentChar'))[1] === 'Merlin') {
                agentTypeOptions = ' \
                    <option value=Rover>똘똘이</option> \
                    <option value=Merlin selected>사이버 마법사</option> \
                    <option value=Bonzi>Bonzi-Buddy</option> \
                    <option value=Rocky>재롱이</option> \
                ';
            } else if(cookies.match(regex_data('agentChar'))[1] === 'Bonzi') {
                agentTypeOptions = ' \
                    <option value=Rover>똘똘이</option> \
                    <option value=Merlin>사이버 마법사</option> \
                    <option value=Bonzi selected>Bonzi-Buddy</option> \
                    <option value=Rocky>재롱이</option> \
                ';
            } else if(cookies.match(regex_data('agentChar'))[1] === 'Rocky') {
                agentTypeOptions = ' \
                    <option value=Rover>똘똘이</option> \
                    <option value=Merlin>사이버 마법사</option> \
                    <option value=Bonzi>Bonzi-Buddy</option> \
                    <option value=Rocky selected>재롱이</option> \
                ';
            } else {
                agentTypeOptions = ' \
                    <option value=Rover>똘똘이</option> \
        ...
        data.innerHTML = ' \
            ...
            <div id=wikiSettings class=skin-settings-page style="padding: 10px; border: 1px solid #ccc; background: white;"> \
                ...
                <div class=form-group> \
                    <label><input ' + set_data["hideStrike"] + ' type="checkbox" id="hideStrike" name="hideStrike" value="hideStrike"> 취소선 숨기기</label> \
                </div> \
                <br /> \
                <div class=form-group> \
            ...
            <div id=siteSettings class=skin-settings-page style="display:none; padding: 10px; border: 1px solid #ccc; background: white;"> \
                ...
                <div class="pull-right form-group"> \
                    <button onclick="location.href = \'/member/mypage\';" type=button>색 구성표 변경</button> \
                </div> \
                <div class=form-group> \
                    <label><input ' + set_data["commonTasksListOnRight"] + ' type="checkbox" id="commonTasksListOnRight" name="commonTasksListOnRight" value="commonTasksListOnRight"> 일반 작업 패널을 오른쪽에 표시</label> \
                </div> \
                <div class=form-group> \
                    <label><input ' + set_data["hideCommonTasksList"] + ' type="checkbox" id="hideCommonTasksList" name="hideCommonTasksList" value="hideCommonTasksList"> 일반 작업 패널 숨기기</label> \
                <div class=form-group> \
                    <label><input ' + set_data["noClippy"] + ' type="checkbox" id="noClippy" name="noClippy" value="noClippy"> 길잡이 끄기</label> \
                </div> \
                <div class=form-group> \
                    <label>길잡이 종류:</label><br> \
                    <select class=form-control id=agentChar name=agentChar>' + agentTypeOptions + '</select> \
                </div> \
                </fieldset></div> \
            </div><div class="btns pull-right"> \
            <button type=reset class="btn btn-secondary form-control">초기화</button> \
            <button onclick="get_post();" class="btn btn-primary form-control" type=button>저장</button> \
            </div></form> \
        ';
